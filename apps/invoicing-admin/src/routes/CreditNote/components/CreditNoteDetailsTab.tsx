  ...
  const creditNoteDetails = () => (
    <>
      <CardTitle tag='h6' className='mb-4'>
        Credit Note: Details
        <span className='small ml-1 text-muted'>#{invoiceId}</span>
      </CardTitle>
      <Form
        style={{
          ...
          <FormGroup row>
            ...
            <Col sm={7}>
              <DatePicker
                ...
                selected={
                  new Date(
                    Date.parse(formatDate(new Date(creditNote?.dateIssued)))
                  )
                }
              />
            </Col>
        ...
        <tbody>
          <tr>
            ...
            <td colSpan={2} className='align-middle text-right'>
              <span>{numeral(creditNote?.price).format('$0.00')}</span>
            </td>
            <td
              colSpan={2}
              className='align-middle text-right text-dark font-weight-bold'
            >
              {numeral(creditNote?.price).format('$0.00')}
            </td>
          </tr>
          <tr>
            <td colSpan={3}></td>
