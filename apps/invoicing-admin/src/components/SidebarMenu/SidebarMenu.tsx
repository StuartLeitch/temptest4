...
class SidebarMenu extends React.Component<SidebarMenuProps, SidebarMenuState> {
  ...
  removeEntry(id: keyof SidebarMenuState['entries']) {
  ...
  setActiveEntries(openActive = false) {
    ...
    const activeChild: any = _.find(this.state.entries, (entry: any) => {
    ...
    if (activeChild) {
      ...
      this.setState({
        entries: (this.entries = _.mapValues(this.entries, (entry: any) => {
