import { ErpData, ErpServiceContract, InvoiceItemType, Payer, Invoice, PayerType, InvoiceItem, Address, Article, Journal } from '@hindawi/shared';
import { Connection, RecordResult, SuccessResult, UserInfo, ErrorResult } from 'jsforce';
...
export class ErpService implements ErpServiceContract {
  ...
  constructor(private config: Config, private fixedValues: ErpFixedValues = defaultErpFixedValues) {
  }
  ...
  async registerInvoice(data: ErpData): Promise<true> {
    ...
    const tradeItemIds = await Promise.all(items.map(
      (item) => this.registerInvoiceItem(tradeDocumentId, data, item)
    ));
  ...
  private async getConnection(): Promise<Connection> {
    ...
    if (!this.connection) {
      ...
      this.loginPromise = this.loginPromise || this.connection.login(user, password + securityToken);
  ...
  private async registerPayer(data: Partial<ErpData>): Promise<string> {
    ...
    const name = payer.type === PayerType.INDIVIDUAL
  ...
  private async registerTradeDocument(accountId: string, data: Partial<ErpData>): Promise<string> {
    const connection = await this.getConnection();
  ...
  private async registerInvoiceItem(
    ...
    const description = invoiceItem.type === 'APC'
  ...
  private get TaxType(payer: Payer): string {
    // TODO datermine this based on payer country and VAT number
  }
