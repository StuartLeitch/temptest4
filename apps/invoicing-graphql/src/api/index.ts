...
export function makeExpressServer(context: Context) {
  ...
  app.post('/api/acceptManuscript', async (req, res) => {
    ...
    const invoiceItems = await context.repos.invoiceItem.getItemsByInvoiceId(InvoiceId.create(new UniqueEntityID(invoiceId)).getValue())
    const manuscript = await context.repos.manuscript.findById(invoiceItems[0].manuscriptId)
                ...
                "files": [
                    {
                        ...
                        "providerKey": "8250aba9-4f34-491b-be13-1edcae7ff4ba/b36c1c6c-c19b-4736-9a80-ccde58c3e7e4"
                    }
                ],
                "editors": [
                    {
