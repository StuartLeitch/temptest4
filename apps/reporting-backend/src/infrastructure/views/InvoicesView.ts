...
AS SELECT
  ...
  JOIN (select event_id from (select event_id, row_number() over (partition by invoice_id order by event_timestamp, updated_date desc) as rn from ${invoiceDataView.getViewName()} id) i where i.rn = 1) last_invoices
