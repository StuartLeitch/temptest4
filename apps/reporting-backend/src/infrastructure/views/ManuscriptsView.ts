...
AS SELECT 
  ...
  concat(e2.given_names, ' ', e2.surname) as academic_editor, e2.email as academic_editor_email, e2.country as academic_editor_country, e2.aff as academic_editor_affiliation,
...
FROM ${submissionView.getViewName()} s 
  ...
  LEFT JOIN LATERAL (SELECT * FROM ${manuscriptEditorsView.getViewName()} e2 where e2.manuscript_custom_id = s.manuscript_custom_id and e2.role_type = 'academicEditor') e2 on e2.manuscript_custom_id = s.manuscript_custom_id
  LEFT JOIN LATERAL (SELECT * FROM ${manuscriptEditorsView.getViewName()} e3 where e3.manuscript_custom_id = s.manuscript_custom_id and e3.role_type = 'editorialAssistant') e3 on e3.manuscript_custom_id = s.manuscript_custom_id
