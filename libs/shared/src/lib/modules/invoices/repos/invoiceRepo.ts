...
export interface InvoiceRepoContract extends Repo<Invoice> {
  ...
  updateAcceptedDate(invoiceId: InvoiceId, acceptedDate: Date): Promise<Either<GuardFailure | RepoError, Invoice>>;
  existsWithId(id: InvoiceId): Promise<Either<GuardFailure | RepoError, boolean>>;
  getInvoicesIds(ids: string[], journalIds: string[], omitDeleted: boolean): AsyncGenerator<string, void, undefined>;
