...
When(/^The Article.create method is called for a given ID "([\w-]+)"$/, function(
...
When(/^I try to mark as published an article with ID "([\w-]+)" on "([\w-]+)"$/, function(
  testArticleId: string,
  datePublished: string
) {
  const articleId = new UniqueEntityID(testArticleId);
  article = Article.create(
    {customId: 'custom-id'}, articleId
  ).getValue();
...
Then(/^The published date for the Article with ID "([\w-]+)" is successfully updated to "([\w-]+)"$/, function(
  testArticleId: string,
  datePublished: string
){
  expect(article.id.toValue()).to.equal(testArticleId);
