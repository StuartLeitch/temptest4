...
Given(/^There is an Invoice with a non-existing ID "([\w-]+)"$/, async function(
  testInvoiceId: string
) {
  return;
});

When(/I try update the status for the Invoice with ID "([\w-]+)" to (.+)/, async function(
  testInvoiceId,
  status: string,
 ){
...
Then(/The Invoice with ID "([\w-]+)" is successfully updated to (.+)/, async function(
  testInvoiceId: string,
  status: string
  ) {
