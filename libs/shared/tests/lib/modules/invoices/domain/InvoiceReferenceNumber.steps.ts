...
Given(
  /^The date of manuscript acceptance is ([^\"]*)$/,
  function (setAcceptanceDate: string) {
...
Given(
  ...
  function (setConfirmationDate: string) {
    invoice.dateIssued = setConfirmationDate
      ? new Date(setConfirmationDate)
      : null;
  }
);

When(
  /^The Invoice changes status to ([\w-]+)$/,
  function (setInvoiceStatus: string) {
...
Then(
  /^Stored reference number should be ([^\"]*)$/,
  function (testPersistentReferenceNumber: string) {
